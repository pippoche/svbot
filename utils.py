...
def safe_callback_data(data: str) -> str:
    return base64.urlsafe_b64encode(data.encode()).decode()[:64]
...
def build_project_keyboard(projects, include_manual=False):
    keyboard = [
        [InlineKeyboardButton(f"{p['Номер договора']} ({p['Ф.И.О заказчика']})", callback_data=safe_callback_data(f"proj_{p['Номер договора']}"))]
...
def build_material_keyboard(materials, mat_inputs=None, show_submit=False):
    ...
    for mat in materials:
        ...
        keyboard.append([InlineKeyboardButton(label, callback_data=safe_callback_data(f"mat_{mat['name']}"))])
...
def build_instrument_keyboard(instruments, selected_instruments, show_submit=True):
    ...
    for instr in instruments:
        ...
        keyboard.append([InlineKeyboardButton(button_text, callback_data=safe_callback_data(f"inst_{name}"))])
